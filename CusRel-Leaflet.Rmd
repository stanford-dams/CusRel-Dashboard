...
the_map <- cus_rel_data %>%
  leaflet() %>%
   ...
   addPolylines(data = website_routes_sf, label = website_routes_sf$PUB_RTE, group="Show Routes", color=palette) %>%
   addCircleMarkers(data = website_stops, popup = website_stops$STP_DESCRI, group = "Show Stops", color = "black", radius = 0.05) %>%
   addLayersControl(
