---
title: "Clean-Data.Rmd"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(tidyverse)
library(readr)
library(gtools)
WORKING_DIR <- "/Users/harrisonli/Documents/iCloud_Documents/Stanford/DAMS/AC_Transit/CusRel-Dashboard"
```


```{r}
raw_data <- read_csv("CusRel-Data.csv")
raw_stops <- read_csv("AC_transit_all_stops.csv")
```

```{r}
# Change StopID in stops data to character, remove irrelevant columns
stops <- raw_stops %>%
  dplyr::select(StopId, Latitude, Longitude) %>%
  mutate(StopId = as.character(StopId))
```


```{r}
# Add lat/lon columns
cleaned_data <- raw_data %>% 
  dplyr::filter(across(Location, ~grepl(pattern=".*\\(\\d\\d\\d\\d\\d\\)$",.))) %>%
  mutate(StopId=substr(Location, nchar(Location)-5, nchar(Location)-1)) %>%
  left_join(stops, by="StopId") %>% # add lat lon to table based on stops data
  tidyr::separate(Reasons, into = c("Reason1", "Reason2"), sep = ";", fill = "right", extra = "drop") %>%
  filter(!is.na(Longitude) & !is.na(Latitude))
```

```{r}
cleaned_data <- cleaned_data %>%
  mutate(ReceivedDateTime=as.POSIXlt(as.character(ReceivedDateTime), tz="PST8PDT"),
         ResolvedDateTime=as.POSIXlt(as.character(ResolvedDateTime), tz="PST8PDT"),
         IncidentDateTime=as.POSIXlt(as.character(IncidentDateTime), tz="PST8PDT"),
         updatedOn=as.POSIXlt(as.character(updatedOn), tz="PST8PDT"))
```



```{r}
# Add Resolve Time, Incident + Receive Date; Create Label for Popup
cleaned_data <- cleaned_data %>% 
  mutate(ResolveTime = round(as.numeric(difftime(as.POSIXct(ResolvedDateTime), as.POSIXct(ReceivedDateTime), units = "days"))), 
         ReceivedDate = as.Date(ReceivedDateTime, "%m/%d/%y", tz="PST8PDT"), 
         IncidentDate = as.Date(IncidentDateTime, "%m/%d/%y", tz="PST8PDT"), 
         Route = toupper(Route)) %>% 
  mutate(Label = str_c("<b>Received Date:</b> ", ReceivedDate, "<br/>",
                       "<b>Incident Date:</b> ", IncidentDate, "<br/>",
                       "<b>Location:</b> ", Location, "<br/>", 
                       "<b>Destination:</b> ", Destination , "<br/>",
                       "<b>Route:</b> ", Route, " &nbsp;&nbsp ", "<b>Vehicle #:</b> ", VehNo, "<br/>", 
                       "<b>Resolve Time:</b> ", ResolveTime, " day(s)<br/>", 
                       "<b>Reason(s):</b> ", Reason1, if_else(is.na(Reason2), "", str_c("; ", Reason2))
                       )) # Add a Label for Popup

```

```{r}
# Change N/A and NULL into Unknown
cleaned_data <- cleaned_data %>%
  mutate(IncidentCity = ifelse((IncidentCity %in% c("NULL", "N/A")), "Unknown", IncidentCity),
         Route = ifelse((Route %in% c("NULL", "N/A") | is.na(Route)), "Unknown", Route),
         ForAction = ifelse(is.na(ForAction), "Unknown", ForAction))
```

```{r}
# 4/28 edits: Sorting the Routes, Reason1/2, and ForAction columns

# Make a copy
copy_cleaned_data <- cleaned_data

# Actually sorting the columns
edited_Routes <- mixedsort(cleaned_data$Route, decreasing = FALSE)
edited_Reason1 <- str_sub(cleaned_data$Reason1, 5)
edited_Reason2 <- str_sub(cleaned_data$Reason2, 5)
edited_ForAction <- if_else(cleaned_data$ForAction == "Unknown", cleaned_data$ForAction, str_sub(cleaned_data$ForAction, 4))

# Replacing corresponding columns in df
copy_cleaned_data[["Route"]] <- edited_Routes
copy_cleaned_data[["Reason1"]] <- edited_Reason1
copy_cleaned_data[["Reason2"]] <- edited_Reason2
copy_cleaned_data[["ForAction"]] <- edited_ForAction
```


```{r}
# Write out
write_csv(cleaned_data, paste(WORKING_DIR, "/Clean-CusRel-data.csv", sep=""))
```

```{r}
# Write out (4/28 edits)
write.csv(copy_cleaned_data, "/Users/cynthiachen/Desktop/School stuff/DAMS/CusRel-dashboard/Clean-CusRel-data.csv")
```



